		...
		methods: {
			...
			handleExportClick() {
				...
				this.$confirm('是否需要导出?', '提示', {
					...
					window.open(`${this.baseURL}terminal/notonlinedc?vehicle=${vehicle}&companyName=${companyName}&area=${area}&stime=${stime && moment(stime).format('YYYY-MM-DD')}&etime=${etime && moment(etime).format('YYYY-MM-DD')}`);
				}).catch(() => {});
			},
