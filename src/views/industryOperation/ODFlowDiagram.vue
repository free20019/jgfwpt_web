		...
		<div class="tw-map-query-panel">
			<template v-if="mapPanel.display">
				...
				<div class="tw-query-panel">
					<el-table :data="table.data" v-loading="table.loading" border size="small" height="calc(100% - 42px)" style="width: 100%; margin-bottom: 10px;" @row-click="test">
		...
		methods: {
			...
			setPolygon(obj,name,num,type,data) {
				...
				if(type == '0'){
					...
					let area;
					if(this.query.startPoint === 0){
						area = "火车东站";
					}
					if(this.query.startPoint === 1){
						area = "城站火车站";
					}
					if(this.query.startPoint === 2){
						area = "萧山机场";
					}
					txt = "起点 "+area+" 至 "+name+" <br>数量："+num+"";
					ys = "#1976d2";
				}
