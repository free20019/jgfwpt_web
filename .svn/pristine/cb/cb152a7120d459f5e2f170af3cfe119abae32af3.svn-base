        ...
        <div class="tw-template-body">
            ...
            <div class="tw-query-panel">
                <el-table :data="filterTableList" v-loading="table.loading" border size="small" height="calc(100% - 42px)" style="width: 100%; margin-bottom: 10px;">
                    ...
                    <el-table-column v-if="index<table.arr.length" v-for="(item,index) in table.arr" :prop="'D'+(index+1)" :label=item align="center" width="120"></el-table-column>
                </el-table>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$nextTick(() => {
                this.query.time = formatYear(moment());
                this.getKeyAreasVehicleQuantityHalfAYearReport();
                window.onresize = () => {
        ...
        methods: {
            /* 获取一个月的天数*/
            getDays(){
                this.query.time = this.query.time && formatYear(this.query.time);
                this.table.arr = [];
            ...
            /*数据接口*/
            getKeyAreasVehicleQuantityHalfAYearReport(){
                const {area, time, type} = this.query;
                if(!time) return this.$message.error('请选择时间！');
                this.table.loading = true;
                this.getDays();
                axios.get('keyArea/getKeyAreasVehicleQuantityHalfAYearReport', {
            ...
            handleExportClick() {
                ...
                this.$confirm('是否需要导出?', '提示', {
                    ...
                    window.open(`${this.baseURL}keyArea/getKeyAreasVehicleQuantityHalfAYearReportExcel?area=${area}&time=${time&&formatYear(time)}&type=${type}`);
                }).catch(() => {});
            },
