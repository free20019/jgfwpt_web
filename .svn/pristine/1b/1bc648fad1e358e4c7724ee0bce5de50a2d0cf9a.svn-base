	...
	<div class="tw-template-wrapper">
		...
		<div class="tw-query-panel">
			<el-table :data="filterTableList" v-loading="table.loading" border size="small" height="calc(100% - 42px)" style="width: 100%; margin-bottom: 10px;">
				...
				<el-table-column prop="COUNT" label="登陆次数" width="140"></el-table-column>
			</el-table>
...
<script>
	...
	export default {
		...
		mounted() {
			this.$nextTick(() => {
				this.query.stime = moment().format('YYYY-MM-DD 00:00:00');//设置stime和etime的默认时间
				this.query.etime = moment().format('YYYY-MM-DD 23:59:59');
				this.getMaintenanceManageStatistics();
			});
		},
		computed: {
			filterTableList() {
		...
		methods: {
			/*接口*/
			getMaintenanceManageStatistics(){
				const {maintenanceStaff,accountNumber,stime,etime} = this.query;
				axios.get('servicePlatformUsage/getMaintenanceManageStatistics', {
					baseURL: this.baseURL,
					params: {
						realName:maintenanceStaff,
						userName:accountNumber,
						stime: stime && moment(stime).format('YYYY-MM-DD HH:mm:ss'),//设置stime和etime时间格式并且进行非空判断
